...
export const addListing = async (req,res) => {
    try {
        ...
        // Geocode to get coordinates [lng, lat]
        const coordinates = await geocodeCityLandmark({ city, landMark });
        if(!coordinates){
            return res.status(400).json({message:"Unable to geocode location. Please verify city and landmark."})
        }
        ...
        return res.status(500).json({message:`AddListing error: ${message}`})
    }
}
...
export const updateListing = async (req,res) => {
    try {
        ...
        // Fetch existing listing to compare city/landMark and preserve geometry
        const existing = await Listing.findById(id)
        if(!existing){
            return res.status(404).json({message:"Listing not found"})
        }

        let newGeometry = existing.geometry;
        const isLocationChanged = (city && city !== existing.city) || (landMark && landMark !== existing.landMark);
        if(isLocationChanged){
            const coords = await geocodeCityLandmark({ city: city || existing.city, landMark: landMark || existing.landMark });
            if(!coords){
                return res.status(400).json({message:"Unable to geocode updated location. Please verify city and landmark."})
            }
            newGeometry = { type:"Point", coordinates: coords };
        }
